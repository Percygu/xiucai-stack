...
description: |
  ...
  Mac 深色代码块、蓝色行内代码)，要求显式提供 2.35:1 的文章专属封面和 80-120 字手写摘要，
  图片自动上传素材库、末尾自动拼接引流尾巴，最后建草稿停在发布键前。
...
把项目里一篇现成的 `.md` 文章，原样转成公众号排版并推到草稿箱。**只做结构与排版转换，不改正文、不生成封面；发布方必须传入上游写作 skill 生成的专属封面和手写摘要。**
...
- 公众号标题：优先使用用户或上游写作 skill 已明确给出的公众号标题；没有单独标题时，直接使用 frontmatter title 并自动去掉前导序号。`llm-interview` 系列必须自动规范为 `面试官：{题目}`，使用中文冒号；即使传入标题已经带有 `面试官：` 或 `面试官:` 也只保留一个规范前缀。该处理只影响公众号草稿标题，不修改网站文章。被上游流程调用时不要再次追问，避免中断自动发布。
...
- 开头头图：自动插 `assets/images/header-banner.png`（每篇同一张）；它只是公众号正文开头的品牌头图，**绝不能充当公众号封面**。
- **专属封面（必做）**：通过 `--cover` 显式传入本篇文章单独生成的 PNG/JPG 图片。封面实际像素比例必须为 **2.35:1**，脚本会读取尺寸校验；禁止传固定网站截图、`header-banner.png`、网页截图或默认白图。
- **摘要（必做）**：根据文章内容重新手写 80-120 字、推荐 90-110 字的公众号摘要，通过 `--digest "…"` 传入。摘要要用问题、冲突或读者收益建立阅读动机，同时准确概括核心内容；禁止截取正文开头、复用 SEO description、写成关键词列表或只写一句空泛介绍。

### Step 2: 先 dry-run 看排版

```bash
cd {skill_dir}/toolkit
python3 publish.py <文章.md> --title "公众号标题" --digest "手写摘要80-120字" \
  --series-index N --cover <2.35:1专属封面> --dry-run
```
...
去掉 `--dry-run` 重跑，其他参数保持完全一致。脚本会：上传正文图片→上传 `--cover` 指定的专属封面→建草稿。
...
- 开头拼 `assets/header.md`（正文品牌头图 + 引导链接），末尾拼 `assets/cta_footer.md` 对应系列的引流尾巴（`{}`/`{index}` 占位符填"第 N 篇"）。正文品牌头图与公众号封面是两个独立概念。
...
- 封面（thumb，公众号强制）只允许使用 `--cover` 指定的 2.35:1 专属封面；不再回退正文首图或默认白图。
...
- 内容零改动；如需改正文或生成封面，由上游写作 skill 完成，不在本 skill 内临时修改。
- `--digest` 与 `--cover` 都是必填参数；缺失、摘要不足 80 字或超过 120 字、封面不存在或比例不是 2.35:1 时必须停止，不得自动兜底。
