...
def upload_default_thumb(access_token: str) -> str:
    """
    Upload a 940x400 white PNG as placeholder cover (2.35:1 ratio for WeChat).
    ...
    png_bytes = make_png(940, 400)
    resp = requests.post(
        "https://api.weixin.qq.com/cgi-bin/material/add_material",
        params={"access_token": access_token, "type": "image"},
        files={"media": ("cover.png", io.BytesIO(png_bytes), "image/png")},
    )
    data = resp.json()
    if "media_id" not in data:
        errcode = data.get("errcode", "unknown")
        errmsg = data.get("errmsg", "unknown error")
        raise ValueError(f"WeChat upload_default_thumb error: errcode={errcode}, errmsg={errmsg}")
