---
title: 8. Agent的记忆机制
description: AI Agent记忆机制教程：深入讲解智能体的短期记忆、长期记忆、工作记忆三大体系，详解上下文窗口管理、向量检索、Session状态管理等核心技术，结合Go语言代码示例带你理解大模型Agent如何"记住"和"回忆"，构建真正有连续性的AI应用。
category:
  - Go Agent
tag:
  - Go Agent
...
**摘要压缩**是一种更聪明的方案：当对话历史变长时，用大模型对早期的对话进行摘要，把 10 轮详细的对话压缩成一段几百 Token 的总结，然后只保留这段总结加上最近几轮的原始对话。这样既节省了 Token，又保留了关键信息。缺点是摘要过程本身会丢失细节，而且需要额外的模型调用。
...
查询: 金额相关的开发规范
  ...
  🔍 检索到 [m4] 相似度=0.6845: 订单模块已经设计完成，包含orders、order_items、payment...
```
