...
total_words = 0
by_month = defaultdict(lambda: [0, 0])  # 月份 -> [篇数, 字数]

for path in files:
    with open(path, encoding="utf-8") as f:
        text = f.read()
    words = len(text)               # 按字符数统计字数
    total_words += words
    month = os.path.basename(path)[:7]  # 文件名前 7 位即年月 2025-06
    by_month[month][0] += 1
    by_month[month][1] += words

print(f"共 {total_count} 篇笔记，合计 {total_words} 字\n")
print("按月份统计：")
for month in sorted(by_month):
    cnt, words = by_month[month]
    print(f"  {month}：{cnt} 篇，{words} 字")
```
