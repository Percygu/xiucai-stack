...
> 画一张干净白底、现代扁平风格的对比示意图，讲清子代理如何隔离上下文。左半边画「主对话」：一个大的上下文窗口长方形（用 Claude 橙色星芒图标标识），如果不派子代理，里面被几十个杂乱的文件块、日志条、搜索结果塞满，显得拥挤（暖灰色表示拥挤浪费）。中间画一个分叉箭头，把「去翻几十个文件」这件事派给右边。右半边画一个独立的「子代理」窗口（用一个不同颜色的小机器人或放大镜图标，配独立边框表示隔离），里面装着那一大堆文件块和搜索结果（绿色，表示在这里消化）。从子代理窗口只引出一条细箭头回到主对话，箭头上挂一个很小的便签写「只回传 2-3 行结论」。对比之下，主对话窗口因此保持清爽、大片留白。配色语义：暖灰=上下文被占满、绿=在隔离窗口里消化、橙=主对话。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者一眼看懂子代理的核心价值——把会污染上下文的探索过程隔离到独立窗口，主对话只收结论。
...
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的三栏卡片对比图，介绍三个内置子代理。左卡「Explore」配蓝色：一个放大镜+文件图标，标注「只读探索代码库」，下面三行小字「模型 Haiku（快而省）」「仅只读工具」「快速/中等/彻底 三档」。中卡「Plan」配紫色：一个蓝图/规划板图标，标注「Plan 模式下调研」，小字「继承主对话模型」「只读」「为出方案收集上下文」。右卡「general-purpose」配橙色：一个多功能瑞士军刀或齿轮+扳手图标，标注「全能：又探索又动手」，小字「全部工具」「多步复杂任务」。每栏顶部一个圆形图标，下面分行列要点。配色语义：蓝=只读探索、紫=规划、橙=全能动手。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者分清三个内置子代理各自的能力边界和适用场景，知道 Claude 在背后是怎么分派的。
...
> 🔴待截图3 —— `/agents` 子代理管理界面
>
> 截图位置：在 Claude Code 里敲 `/agents`，进入 Library 标签
> 截图内容：子代理管理界面，能看到内置代理列表、新建代理入口、以及创建时选择工具/模型的步骤
> 标注要求：用红框框出「新建代理」入口和工具选择区
> 建议保存为：../../../assets/img/vibe_coding/tools/claude_code/claude_code_subagents/claude_code_subagents3.png
...
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的「标注式代码卡片」图，展示一个子代理 markdown 文件的结构。中央画一个带文件头的代码卡片，分上下两块：上块是 YAML frontmatter（浅黄底），逐行列出 `name`、`description`、`tools`、`model` 四个字段；下块是 markdown 正文（浅蓝底）写「系统提示：你是一名资深代码审查员…」。从每个关键字段向右引出一条带圆点的注释引线，用小标签解释：`name`→「唯一标识，必填」；`description`→「Claude 据此决定何时委派，必填」；`tools`→「限定能用的工具，不写=全继承」；`model`→「指定模型，可填 haiku 省钱」；正文块→「成为子代理的系统提示，塑造它的行为」。注释标签用浅灰底。配色语义：黄=配置、蓝=系统提示、引线灰=说明。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者看懂一个子代理文件由哪几部分组成、每个 frontmatter 字段是干什么的，照着就能手写一个。
...
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的上下对比图。上半部分「前台运行」配蓝色：画一条直线时间轴，主对话（橙色星芒）派出一个子代理后，主对话停下来等待（画一个沙漏/等待图标），子代理干完返回结果，主对话才继续。标注「你等它干完、看结果，默认方式」。下半部分「后台运行（background: true）」配绿色：同样一条时间轴，主对话派出子代理后，主对话这条线继续往前推进别的任务（画连续的工作图标），子代理在下面一条平行线上独立推进，干完用一个铃铛/通知图标提醒。标注「主对话不必干等，子代理后台推进，完成时通知」。右侧小字补充「用 /agents 的 Running 标签或 /tasks 监看」。配色语义：蓝=同步等待、绿=异步并行。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者分清子代理前台（等结果）和后台（边干别的边等通知）两种运行方式，知道耗时任务该丢后台。
...
> 【建议配图6 —— Subagent 星型 vs Agent Teams 网状】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的左右拓扑对比图。左边「Subagent（星型）」配蓝色：中央一个主代理节点（Claude 橙色星芒），周围三四个子代理节点，每个子代理只有一条线单独连回中央主代理（箭头双向但只连中心），子代理彼此之间没有连线。底部标注「主代理统一派发，子代理各自汇报，互不通信」。右边「Agent Teams（网状）」配绿色：一个 Team Lead 节点在上方，下面三四个 teammate 节点彼此之间都有连线（网状互联），中间放一个共享的「TaskList 任务板」图标，所有成员都连向它；成员之间的连线上画小信封图标表示消息往来。底部标注「成员自主认领任务、直接互相通信」。中间用一条竖虚线分隔，顶部各放标题。配色语义：蓝=星型集中、绿=网状协作。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者一眼看清两种多代理模式的拓扑差异——子代理是向心汇报的星型，Agent Teams 是成员互联、共享任务板的网状。
...
> 【建议配图7 —— 该不该用 Agent Teams 的决策图】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的纵向决策流程图。顶部起点框（蓝色）「我有一个任务要做」。向下第一个菱形判断（橙色）「能拆成多个相对独立的子任务吗？」，左侧「否」箭头指向灰色结果框「用单实例（或普通子代理）就好」。「是」向下到第二个菱形「这些子任务能分给不同文件/目录，互不踩脚吗？」，「否」箭头指向浅灰框「考虑串行处理或再拆分」。「是」向下到第三个菱形「多花几倍 Token 成本可以接受吗？」，「否」指向灰框「用单实例」。「是」指向最醒目的绿色结果框「用 Agent Teams ✓」，配一个多人团队图标。每个判断节点用橙色菱形，结果框按推荐度用绿/灰区分。配色语义：蓝=起点、橙=判断、绿=推荐、灰=不必。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：给读者一套清晰的判断标准，避免对简单任务滥用 Agent Teams 白白烧 Token，也不错过它真正能提效的场景。
