...
> 【建议配图1 —— 提示 vs Hook 的确定性差异】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的左右对比图。左半「写在 CLAUDE.md 里的提示」配橙黄色（不确定）：画一个 Claude 橙色星芒图标，旁边一条对话气泡写「记得改完要格式化」，向下分出两条岔路箭头——一条绿色对勾「这次记得了」、一条红色叉「上下文一长，忘了」，用问号图标强调结果不稳定。右半「挂成 Hook」配绿色（确定）：画一个齿轮+闪电图标代表事件触发，一条直箭头从「文件改动事件」笔直指向「自动执行格式化」，结果只有一个绿色对勾，旁边标「每次必执行，与模型状态无关」。中间一条竖虚线分隔。配色语义：橙黄=靠自觉不稳、绿=确定执行。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者一眼理解 Hook 的核心价值——把「希望模型记得」变成「事件触发就必然执行」的确定性保证。
...
> 【建议配图2 —— Claude Code 一次交互的生命周期与可挂钩点】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的横向时间轴流程图，展示一次 Claude Code 交互的生命周期，并在每个节点标出可挂钩事件。时间轴从左到右依次经过这些节点，每个节点一个圆形图标 + 事件名标签：①「会话开始」配 `SessionStart`（电源图标，灰色）；②「你提交消息」配 `UserPromptSubmit`（对话气泡图标，蓝色）；③「Claude 要调用工具前」配 `PreToolUse`（盾牌图标，红色，并用红色小字标「可拦截」）；④「工具调用成功后」配 `PostToolUse`（齿轮图标，绿色）；⑤「Claude 答完一轮」配 `Stop`（旗帜图标，紫色）；⑥「会话结束」配 `SessionEnd`（关机图标，灰色）。节点之间用粗箭头顺序相连。在 `PreToolUse` 上方额外画一个分叉小图示：盾牌可以让请求「放行→继续」或「拦截→拒绝」。配色语义：红=可拦截的安全关口、绿=收尾动作、蓝=输入处理、紫/灰=轮次与会话边界。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者建立「一次交互有哪些时刻、各能挂什么 hook」的全局地图，尤其看清 PreToolUse 是唯一能拦截动作的安全关口。
...
> 【建议配图3 —— Hook 配置的三层嵌套结构】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的「标注式 JSON 卡片」图，拆解 hook 配置的三层结构。中央画一段 JSON 代码卡片（等宽字体），用三个由外到内、不同颜色的圆角边框框住三个层级：最外层框（蓝色）框住 `"PostToolUse"`，右侧引线标注「①事件名：什么时刻触发」；中间层框（橙色）框住 `"matcher": "Edit|Write"`，引线标注「②匹配器：只在哪些工具上触发」；最内层框（绿色）框住 `{ "type": "command", "command": "prettier" ... }`，引线标注「③要执行的命令」。三个引线标签用浅灰底。底部一行小字：从外到内 = 何时 → 哪些工具 → 做什么。配色语义：蓝=事件、橙=过滤、绿=执行。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者看懂 hook 配置「事件→匹配器→命令」的三层嵌套，照着就能改出自己的 hook。
...
> 【建议配图4 —— Hook 的输入输出与放行/拦截决策】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的横向数据流图。左边「Claude 触发事件」节点（橙色星芒），一条箭头标「stdin 喂入 JSON」指向中间的「你的 Hook 脚本」节点（齿轮图标，方框），箭头上挂一个小 JSON 标签写 `tool_name / tool_input / cwd`。中间脚本节点向右分出两条结果箭头到右侧：上面一条绿色箭头标「退出码 0」指向「放行，动作继续」（绿色对勾）；下面一条红色箭头标「退出码 2」指向「拦截，动作被拒」（红色叉），并补一句小字「stderr 内容反馈给 Claude」。脚本节点下方再引一条虚线指向一个小卡片「或打印 JSON 做精细控制：deny / additionalContext」。配色语义：绿=放行、红=拦截、橙=事件源。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：让读者理解 hook 既能读到事件数据、又能用退出码和 JSON 输出决定放行还是拦截，这是写安全校验类 hook 的关键机制。
...
> 🔴待截图5 —— PreToolUse hook 拦截危险命令的实际效果
>
> 截图位置：配好 block-rm.sh 这个 PreToolUse hook 后，让 Claude 尝试执行一条含 `rm -rf` 的命令
> 截图内容：Claude Code 里这次 Bash 调用被 hook 拦截、显示拒绝理由的画面
> 标注要求：用红框框出被拦截的命令和拒绝提示
> 建议保存为：../../../assets/img/vibe_coding/tools/claude_code/claude_code_hooks/claude_code_hooks5.png
...
> 【建议配图6 —— 三类实战 hook 在生命周期上的位置】
>
> **生图提示词（可直接发给 ChatGPT / 文生图工具）：**
> 画一张干净白底、现代扁平风格的横向流程图，把本篇四个实战 hook 标注到一次交互的时间轴上，强化「在不同时刻挂不同 hook」的整体观感。一条从左到右的时间轴，四个挂钩点用不同颜色的标记气泡从轴上引出：①在「你提交消息」处挂一个蓝色气泡「UserPromptSubmit → 注入 git 分支等上下文」；②在「执行 Bash 前」处挂一个红色盾牌气泡「PreToolUse → 拦截 rm -rf 等危险命令」；③在「改完文件后」处挂一个绿色气泡「PostToolUse → 自动 prettier 格式化」；④在「答完一轮」处挂一个紫色气泡「Stop → 自动跑测试，失败接着修」。每个气泡里配一个对应的小图标（对话/盾牌/刷子/烧瓶）。时间轴节点用圆点，气泡用细引线连到对应节点。配色语义：蓝=输入、红=安全拦截、绿=收尾、紫=完成检查。图片右下角放置引流信息：公众号：IT杨秀才 ｜ https://golangstar.cn（小字、浅灰、不抢主体）。
>
> 整体目的：把四个实战 hook 收拢到一张时间轴上，让读者建立「该自动化的事，就挂到它该发生的那个时刻」的整体方法论。
