---
title: 2. Cursor 四大模式完全指南
description: Vibe Coding教程第23篇：Cursor教程四大模式指南。讲清 Ask、Agent、Plan、Edit 的用途边界、模式切换、Cmd+K 局部编辑和复杂任务工作流。
category:
  - Vibe Coding
tag:
  - Vibe Coding
...
![四种入口的放手程度](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes1.png)
## **2. 模式切换**
...
![Cursor 模式选择器](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes2.png)
...
![Cursor Ask 模式只读提问](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes3.png)
...
这类写法能让 Ask 输出更稳定，也为后续是否进入 Plan 或 Agent 提供判断依据。
...
这种问法不会把 Cursor 推进执行阶段，却能把下一步要做什么讲清楚。对小白来说，Ask 常常不是用来问最终答案，而是用来补齐上下文、发现缺口、整理更好的下一轮 Prompt。
...
![Cursor Agent 模式任务输入](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes4.png)
...
这其实是把 Agent 当成会使用工具的研究员，而不是一上来就让它写代码。等调查结果清楚后，再开第二轮让它执行修改。复杂问题分两轮，通常比一轮大 Prompt 更稳。
...
![Cursor Plan 模式任务准备](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes5.png)
...
好的 Plan 应该让你一眼看出接下来会发生什么。看不懂、太宽、太抽象，都不应该放行。特别是对零基础读者来说，Plan 模式最大的意义不是显得专业，而是把改动拆成能理解、能检查、能回退的步骤。
...
![Plan 模式流程](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes6.png)
## **6. Edit 模式**
...
![Cursor Cmd+K 局部编辑](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes7.png)
...
如果你发现自己在 `Cmd+K` 里写了很多范围说明，比如要它读取多个文件、检查项目结构、运行测试，那就说明这个任务已经超出局部编辑，应该切到 Agent 或 Plan。
...
这些 Prompt 都有共同点：范围来自选区，任务是单一动作，限制写得很具体。局部编辑的目标不是让 AI 自由发挥，而是让它在你圈定的区域内做一次高质量改写。
...
![Cursor 复杂任务工作流](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes8.png)
## **8. 模式选择**
...
不要被模式名字吓住。Cursor 的模式选择，本质是普通开发流程的拆分：读代码、定方案、改代码、验收。传统开发里这些步骤也存在，只是以前都由人自己完成；Cursor 把其中一部分交给 AI，但你仍然要决定什么时候读、什么时候计划、什么时候执行。

![Cursor 模式决策树](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes9.png)
## **9. 验收回退**
...
![Cursor 执行后的 README 结果](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes10.png)
...
![终端验证 Cursor 修改结果](../../../assets/img/vibe_coding/tools/cursor/cursor_modes/cursor_modes11.png)
...
这样做不会保证 AI 永远正确，但会让它的汇报更容易检查。你也能更快发现缺口：它没跑测试、范围超了、解释含糊、还有待确认项。Cursor 的高效不是跳过验收，而是让执行更快、验收更清楚。
